...
class Kernel(object):
    ...
    def __call__(self, data_1, data_2):
        ...
        if np.ndim(data_1)==1 and np.ndim(data_2) ==1:
            data_1 = data_1[None,:]
            data_2 = data_2[None,:]
            return self._compute(data_1, data_2)[0,0]
        elif (np.ndim(data_1) > 1 and np.ndim(data_2) == 1):
        	data_2 = data_2[None,:]
        	return self._compute(data_1, data_2).reshape(-1)
        elif (np.ndim(data_1) == 1 and np.ndim(data_2) > 1):
        	data_1 = data_1[None,:]
        	return self._compute(data_1, data_2).reshape(-1)
        else:
